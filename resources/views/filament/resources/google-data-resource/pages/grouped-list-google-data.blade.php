@php
    $groupedData = $this->getGroupedGoogleData();
@endphp
<x-filament-panels::page>
    <div class="space-y-6">
        @foreach($groupedData as $rangeKey => $items)
            <div class="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden" x-data="{ open: false }" wire:key="google-data-{{ md5($rangeKey) }}">
                <div 
                    @click="open = !open"
                    role="button"
                    tabindex="0"
                ...
                >
                    <div class="font-medium">
                        ...
                        <span class="text-sm text-gray-500 dark:text-gray-400 ml-2">
                            ({{ $items->count() }} {{ Str::plural('row', $items->count()) }})
                        </span>
                        @if(!empty($typeRaw))
                    ...
                    <div class="flex items-center">
                        <span class="text-gray-500 dark:text-gray-400 mr-2">
                            ${{ number_format($items->sum('cost'), 2) }} cost
                        </span>
