@php
    $groupedData = $this->getGroupedBinomGoogleSpentData();
@endphp
<x-filament-panels::page>
    <div class="space-y-6">
        @foreach($groupedData as $rangeKey => $items)
            <div class="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden" x-data="{ open: false }" wire:key="binom-google-spent-{{ md5($rangeKey) }}">
                <div 
                    @click="open = !open"
                    role="button"
                    tabindex="0"
                    ...
                    data-qa="binom-google-spent-header"
                >
                    <div class="font-medium">
                        ...
                        <span class="text-sm text-gray-500 dark:text-gray-400 ml-2">
                            ({{ $items->count() }} {{ \Illuminate\Support\Str::plural('entry', $items->count()) }})
                        </span>
                    </div>
                    <div class="flex items-center">
                        <span class="text-gray-500 dark:text-gray-400 mr-2">
                            ${{ number_format($items->sum('revenue'), 2) }} revenue
                        </span>
